...
class ServiceRuleAdmin(admin.ModelAdmin):
    list_display = ['target_service', 'order', 'label', 'service', 'event', 'system_instruction', 'operand_service', 'entity_content_type', 'entity_object_id', 'parameter_values', 'id']
    list_display_links = ['order', 'label']
    search_fields = ['label', 'name', 'pym']
    autocomplete_fields = ['service', 'operand_service', 'target_service']
